...
export function AuthFlowDiagram() {
  ...
  return (
    <div ref={ref} className="space-y-10">
      {/* Prose */}
      <p className="text-sm text-white/55 leading-[1.85]">{authFlow.description}</p>
      ...
      <div>
        ...
        <div className="relative">
          {/* Vertical line */}
          <div className="absolute left-3 top-3 bottom-3 w-px bg-white/[0.07]" aria-hidden="true" />
            ...
            {authFlow.steps.map((s, i) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-mono font-semibold text-white/80 mb-0.5">{s.step}</div>
                  <div className="text-xs text-white/40 leading-relaxed">{s.detail}</div>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-px bg-white/[0.06] rounded-sm overflow-hidden">
          <div className="bg-[#080808] px-5 py-3 border-b border-white/[0.06]">
            <span className="text-[10px] font-mono text-emerald-400/60 uppercase tracking-widest">Advantages</span>
          </div>
          <div className="bg-[#080808] px-5 py-3 border-b border-white/[0.06]">
            <span className="text-[10px] font-mono text-orange-400/60 uppercase tracking-widest">Trade-offs</span>
          </div>
            ...
            <>
              ...
              >
                <div className="flex items-start gap-2">
                  <span className="text-emerald-400/50 mt-0.5 text-xs shrink-0">+</span>
                  <span className="text-xs text-white/45 leading-relaxed">{t.pro}</span>
                </div>
              </motion.div>
              <motion.div
                key={`con-${i}`}
              ...
              >
                <div className="flex items-start gap-2">
                  <span className="text-orange-400/50 mt-0.5 text-xs shrink-0">−</span>
                  <span className="text-xs text-white/45 leading-relaxed">{t.con}</span>
                </div>
              </motion.div>
            </>
          ))}
      ...
      <div>
        ...
        <div className="space-y-2">
          {authFlow.whyJWT.map((reason, i) => (
            ...
            >
              <span className="w-1 h-1 rounded-full bg-[#C5A880]/50 mt-1.5 shrink-0" aria-hidden="true" />
              <span className="text-xs text-white/50 leading-relaxed">{reason}</span>
            </motion.div>
          ))}
        </div>
      </div>
